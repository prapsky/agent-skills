...
2. Read each matching skill’s `SKILL.md` (and `reference.md` when linked). Prefer skills that touch the PR surface, for example:
   ...
   - `local-docker-mysql` / `local-docker-redis` / `local-docker-dynamodb` / `local-docker-firestore` / `local-docker-pubsub` / `local-docker-firebase-remote-config` — only if the PR adds/changes those local-store skills or Docker seed recipes
